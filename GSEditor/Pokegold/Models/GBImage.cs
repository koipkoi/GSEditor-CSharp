...
public sealed class GBImage
{
  ...
  private static readonly byte[] _paletteHeader = new byte[] { 0x50, 0x4c, 0x54, 0x45, };
  ...
  private static Color[]? DecodePalette(string filename)
  {
    ...
    for (var i = 0; i < 4; i++)
    {
      result[i] = Color.FromArgb(255, bytes[index + 0], bytes[index + 1], bytes[index + 2]);
      index += 3;
    }
    ...
    try
    {
      ...
      for (var i = 0; i < Source.Length; i += 2)
      {
        ...
        for (var pixelX = 0; pixelX < 8; pixelX++)
        {
          ...
          bitmap.SetPixel(x + pixelX, y, Colors[colorIndex]);
        }
      }
